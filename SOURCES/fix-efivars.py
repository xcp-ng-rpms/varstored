...
def get_pool_ref(session, pool_uuid: Optional[str]):
    if pool_uuid:
...
def get_pool_certs_type(session, pool_ref: Any, custom=True):
    ...
    with tarfile.open(fileobj=cert_buf) as cert_file:
        for member in cert_file:
            ...
            if member.size > TOTAL_LIMIT:
                raise ValueError(f"Certs member '{varname}' too big")
...
def get_pool_certs(session, pool_ref: Any):
...
def do_scan_pool(session, *, pool_ref: Any):
    ...
    pool_certs = get_pool_certs(session, pool_ref)
