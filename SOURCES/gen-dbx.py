...
# Reimplementation of DBX generation using schema from https://github.com/microsoft/secureboot_objects
...
def make_efi_auth_variable(
    ...
    attributes: int,
    append: bool,
    signer_cert: typing.Union[str, pathlib.Path],
    signer_key: typing.Union[str, pathlib.Path],
    tmpdir: str,
):
...
def convert_certificate(infile, outfile):
    ...
    for inform in cert_forms:
        ...
        raise Exception("Cannot convert certificate file")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--signer-cert", required=True, type=pathlib.Path, help="Path of signer certificate")
    parser.add_argument("--signer-key", required=True, type=pathlib.Path, help="Path of signer private key")
    ...
    with args.input as ifile, tempfile.TemporaryDirectory() as tmpdir:
        ...
        if "svns" in sets:
            siglist_svns = []
            svns: typing.Optional[typing.List[dict]] = data.get("svns", [])
            for svn in svns:
